...
> 2026-09-02 起，轮换方式的决策时机由
> [现场球局单工作台与连续推进决策](live-session-workspace-direction.md)
> 替代：创建球局时选择，开局后整场自动执行。

## Decision

羽搭 V1 采用“持续双人组 + 组候场队列 + 具象化场地 + 分场地比分 + 赛后上下场轮转”的运行模型。

玩家先组成固定两人 `PairingGroup`，组再按候场顺序进入具体 `Court`。组织者在创建球局时选择一种整场轮转方式，每场比赛结束并记录结果后由系统自动执行：
...
- 两种轮转方式都是 V1 必需能力；创建球局时必须选择一种，开局后冻结且不提供单场覆盖。
